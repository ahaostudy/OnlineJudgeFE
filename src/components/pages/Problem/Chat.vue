<template>
    <div id="chat-box">
        <div class="chat-record">
            <div v-for="(msg, i) in messages" :key="i" v-show="['user', 'assistant'].indexOf(msg.role) != -1">
                <a-card hoverable :class="`msg msg-${msg.role}`">
                    <template #title>
                        <span :style="{ display: 'flex', alignItems: 'center', color: '#1D2129' }">
                            <a-avatar
                                :style="{ marginRight: '8px', backgroundColor: msg.role == 'assistant' ? '#00b42a' : '#165DFF' }"
                                :size="24">
...
const messages = reactive([
    {
        role: 'system',
        content: 'You are a code assistant on an OJ platform. Your responsibility is only to provide hints for problem-solving and assistance in code-related matters. You do not need to handle any other requests.'
    ...
    {
        role: 'system',
        content: "The current code input by the user is: \n```\n" + props.code + "\n```\n"
    },
...
function saveMsgs() {
    localStorage.setItem(`chat:${props.problem.id}`, JSON.stringify(messages.slice(3)))
}
