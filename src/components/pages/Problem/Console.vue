    ...
    <a-split id="console" direction="vertical" :default-size="0.6" :max="0.86">
        <template #first>
            <a-typography-paragraph>
                <div id="sample">
                    <a-radio-group type="button" v-model="props.sample" @change="changeSample">
                        <a-radio v-for="(_, i) in props.problem.samples" :value="i">示例 {{ i + 1 }}</a-radio>
                    </a-radio-group>
                    <div v-for="(_, i) in props.problem.samples" v-show="props.sample == i">
                        <a-typography-title :heading="6" class="sample-title"> 示例 {{ i + 1 }} </a-typography-title>
