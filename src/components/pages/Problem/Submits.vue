  ...
  >
    <template #first>
      ...
      <div class="preview-box" v-show="!loading && !empty && !editing">
        ...
        <div class="preview">
          <v-md-preview
            :text="submit.note_id ? submit.note.content : submitInfo"
          />
        </div>
      </div>
...
for (const key in constStore.Status) {
  if (
    key == constStore.StatusHide ||
    key == constStore.StatusFinished ||
    key == constStore.StatusServerFailed
  )
    continue
  statuses.push({
    value: key,
...
function flushTable() {
  ...
  getSubmits(props.id).then((res) => {
    ...
    const errors = [
      constStore.StatusCompileError,
      constStore.StatusRuntimeError,
      constStore.StatusWrongAnswer,
      constStore.StatusServerFailed
    ]
    const warnings = [
      constStore.StatusTimeLimitExceeded,
      constStore.StatusMemoryLimitExceeded,
      constStore.StatusOutputLimitExceeded
    ]
    for (let s of res.submit_list) {
      ...
      else if (warnings.indexOf(s['status'].code) !== -1)
        s['status'].color = 'orange'
...
function saveNote() {
  ...
  if (!submit.note_id)
    ...
    putUpdateNote(submit.note_id, submit.note.title, submit.note.content).then(
      callback
    )
}
